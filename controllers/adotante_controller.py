...
class AdotanteController:
  ...
  def incluir_adotante(self):
    ...
    data_nascimento_formatada = datetime.strptime(dados_adotante["data_nascimento"], "%d/%m/%Y")
    
    adotante = Adotante(dados_adotante["cpf"], dados_adotante["nome"], data_nascimento_formatada.date(), dados_adotante["logradouro"], dados_adotante["numero"], TipoHabitacao(dados_adotante["tipo_habitacao"]), TamanhoHabitacao(dados_adotante["tamanho_habitacao"]), True if dados_adotante["possui_animal"] == 1 else False)
  ...
  def alterar_adotante(self):
    ...
    if(adotante is not None):
      ...
      adotante.tamanho_habitacao = TamanhoHabitacao(novos_dados_adotante["tamanho_habitacao"])
      adotante.possui_animal = True if novos_dados_adotante["possui_animal"] == 1 else False
      adotante.add_endereco(novos_dados_adotante["logradouro"], novos_dados_adotante["numero"])
      self.listar_adotantes()
    else:
  ...
  def listar_adotantes(self):
    for adotante in self.__adotantes:
      self.__tela_adotante.mostra_adotante({"cpf": adotante.cpf, "nome": adotante.nome, "data_nascimento": adotante.data_nascimento, "tipo_habitacao": adotante.tipo_habitacao, "tamanho_habitacao": adotante.tamanho_habitacao, "possui_animal": adotante.possui_animal, "endereco": adotante.endereco})
